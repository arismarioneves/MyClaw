...
╚═╝     ╚═╝   ╚═╝        ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝${c.reset} (by Mario)
  ...
  const lines: string[] = [
    `# Generated by MyClaw setup — ${new Date().toISOString()}`,
    '',
    '# ─── Messengers ─────────────────────────────────────────────────────────────',
...
`
      ...
      try {
        execSync('systemctl --user daemon-reload && systemctl --user enable myclaw && systemctl --user start myclaw', { shell: true })
